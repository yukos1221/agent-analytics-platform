...
const specPath = path.resolve(__dirname, '../../../../specs/openapi.mvp.v1.yaml');

let spec: SwaggerParser.Document;
let ajv: Ajv;

// Create a test JWT for requests
function createTestJwt(orgId: string = 'org_default', userId: string = 'user_test') {
...
function getResponseSchema(path: string, method: string, statusCode: string): unknown {
    ...
    if (!content) {
        throw new Error(`No application/json content for ${method} ${path} ${statusCode}`);
    }
 ...
 */
function validateResponse(schema: unknown, body: unknown): { valid: boolean; errors: string[] } {
...
describe('OpenAPI Contract Tests', () => {
    beforeAll(async () => {
        ...
        spec = (await SwaggerParser.dereference(specPath)) as SwaggerParser.Document;
    ...
    describe('GET /v1/metrics/overview', () => {
        ...
        it('validates response headers match spec', async () => {
            ...
            if (rateLimitRemaining !== null) {
                expect(Number.parseInt(rateLimitRemaining, 10)).toBeGreaterThanOrEqual(0);
            }
            if (rateLimitReset) {
