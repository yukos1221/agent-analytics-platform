...
import { API_KEYS, createEvent, generateSessionId, generateEventId } from '../fixtures/events';
import { MetricsOverviewResponseSchema, ErrorResponseSchema } from '../../src/schemas';
...
async function seedEvents(events: unknown[]): Promise<void> {
    await eventStore.ingest(METRICS_ORG_ID, events as ReturnType<typeof createEvent>[]);
}

// Create a test JWT for metrics requests
function createTestJwt(orgId: string = METRICS_ORG_ID, userId: string = 'user_test') {
    const secret = process.env.JWT_HS256_SECRET || 'testsecret';
...
describe('Metrics Overview Integration', () => {
    ...
    describe('Multi-Tenant Isolation (CRITICAL)', () => {
        it('metrics only include events from the metrics org', async () => {
            ...
            await eventStore.ingest(
                'org_other',
                createSessionEvents('user_other_org') as ReturnType<typeof createEvent>[]
            );
    ...
    describe('Golden Dataset Verification', () => {
        ...
        it('Success rate: 8 success + 2 error = 80%', async () => {
            ...
            for (let i = 0; i < 8; i++) {
                allEvents.push(...createSessionEvents(`user_success_${i}`, { hasError: false }));
            }

            // 2 failed sessions
            for (let i = 0; i < 2; i++) {
                allEvents.push(...createSessionEvents(`user_error_${i}`, { hasError: true }));
            }
