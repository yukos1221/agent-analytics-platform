...
// Create a test JWT for sessions requests
function createTestJwt(orgId: string = SESSIONS_ORG_ID, userId: string = 'user_test') {
...
async function seedEvents(events: unknown[]): Promise<void> {
    await eventStore.ingest(SESSIONS_ORG_ID, events as ReturnType<typeof createEvent>[]);
}
...
describe('Sessions API Integration Tests', () => {
    ...
    describe('GET /v1/sessions', () => {
        ...
        it('sorts sessions by started_at descending by default', async () => {
            ...
            const sessionIds = body.data.map((s: { session_id: string }) => s.session_id);
            expect(sessionIds.indexOf(sessionId2)).toBeLessThan(sessionIds.indexOf(sessionId1));
        });
    });
