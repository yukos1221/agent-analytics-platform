...
import { API_KEYS, createEvent, generateSessionId, createSessionEvents } from '../fixtures/events';
import { MetricsTimeseriesResponseSchema, ErrorResponseSchema } from '../../src/schemas';
...
// Create a test JWT for timeseries requests
function createTestJwt(orgId: string = TIMESERIES_ORG_ID, userId: string = 'user_test') {
...
async function seedEvents(events: unknown[]): Promise<void> {
    await eventStore.ingest(TIMESERIES_ORG_ID, events as ReturnType<typeof createEvent>[]);
}
...
describe('Timeseries API Integration Tests', () => {
    ...
    describe('GET /v1/metrics/timeseries', () => {
        ...
        it('calculates success_rate metric correctly', async () => {
            ...
            const completedEvents = createSessionEvents(completedSessionId).map((e, i) => ({
                ...e,
                timestamp: new Date(now - i * 1000).toISOString(),
            }));
        ...
        it('includes aggregations when data exists', async () => {
            ...
            if (body.data.length > 0 && body.aggregations) {
                ...
                expect(body.aggregations.min).toBeLessThanOrEqual(body.aggregations.max);
            }
        });
