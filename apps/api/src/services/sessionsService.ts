...
import { querySessionsByOrgRange, queryEventsByOrgRange } from '../../../../packages/database/src';
import type {
    SessionListQuery,
    SessionSummary,
...
function calculateSessionMetrics(events: StoredEvent[]): {
    ...
    for (const event of events) {
        ...
        if (event.event_type === 'task_complete') {
            ...
            const duration = (event.metadata as { duration_ms?: number })?.duration_ms;
            if (duration) {
    ...
    const avg_task_duration_ms =
        taskDurations.length > 0
            ? Math.round(taskDurations.reduce((sum, d) => sum + d, 0) / taskDurations.length)
            : undefined;
...
export async function listSessions(
    ...
    const dbSessions = await querySessionsByOrgRange(orgId, filterStart, filterEnd);
    if (dbSessions !== null && Array.isArray(dbSessions)) {
        ...
        for (const [sessionId, events] of sessionMap.entries()) {
            ...
            const sessionEndEvent = sortedEvents.find((e) => e.event_type === 'session_end');
            const endedAt = sessionEndEvent ? new Date(sessionEndEvent.timestamp) : null;
    ...
    if (query.status && query.status.length > 0) {
        filteredSessions = filteredSessions.filter((s) => query.status!.includes(s.status));
    }

    if (query.agent_id) {
        filteredSessions = filteredSessions.filter((s) => s.agent_id === query.agent_id);
    }

    if (query.user_id) {
        filteredSessions = filteredSessions.filter((s) => s.user_id === query.user_id);
    }
    ...
    // Helper function to generate user info from user_id
    const getUserInfo = (userId: string) => {
        // For MVP, generate fake user info based on user_id
        // In production, this would come from a users database
        ...
        const domains = ['company.com', 'tech.com', 'ai.com'];

        return {
            name: names[Math.abs(userIdHash) % names.length],
            email: `${userId.slice(-4).toLowerCase()}@${
                domains[Math.abs(userIdHash) % domains.length]
            }`,
        };
    };
    ...
    const data: SessionSummary[] = paginatedSessions.map((session) => {
        ...
        const userInfo = getUserInfo(session.user_id);
        ...
        return {
            ...
            ended_at:
                session.ended_at instanceof Date
                    ...
                    : null,
            duration_seconds: session.duration_ms ? Math.floor(session.duration_ms / 1000) : null,
            metrics: {
...
export async function getSessionDetail(
    ...
    if (dbSessions !== null && Array.isArray(dbSessions)) {
        sessionRow = (dbSessions as unknown as SessionRow[]).find((s) => s.id === sessionId);
    }

    // If not found in DB, try to derive from events
    if (!sessionRow) {
        const allEvents = eventStore.getByOrg(orgId);
        const sessionEvents = allEvents.filter((e: StoredEvent) => e.session_id === sessionId);
        ...
        const startedAt = new Date(firstEvent.timestamp);
        const endedAt =
            lastEvent.event_type === 'session_end' ? new Date(lastEvent.timestamp) : null;
    ...
    const timeline = {
        ...
        last_event:
            sessionEvents.length > 0
                ? new Date(sessionEvents[sessionEvents.length - 1].timestamp).toISOString()
                : undefined,
    };

    // Extract client info from metadata if available
    const clientInfo = sessionEvents.find((e) => e.metadata && typeof e.metadata === 'object')
        ?.metadata as
        | {
              client_info?: {
                  ide?: string;
    ...
    return {
        ...
        ended_at:
            sessionRow.ended_at instanceof Date
                ...
                : null,
        duration_seconds: sessionRow.duration_ms ? Math.floor(sessionRow.duration_ms / 1000) : null,
        metrics: {
            tasks_completed: metrics.tasks_completed,
            tasks_failed: metrics.tasks_failed,
