	...
	switch (event_type) {
		...
		case 'tool_call':
			return metadata?.tool_name
				? `Called tool: ${metadata.tool_name}`
				: 'Tool called';
		case 'tool_response':
			return metadata?.tool_name
				? `Tool ${metadata.tool_name} responded`
				: 'Tool responded';
		case 'error':
			return metadata?.error_message
									...
									{event.metadata && Object.keys(event.metadata).length > 0 && (
										<div className="mt-2 text-xs text-gray-600 bg-gray-50 p-2 rounded">
											{Object.entries(event.metadata)
												.filter(([key]) => key !== 'error_message' && key !== 'message' && key !== 'reason') // These are already in description
