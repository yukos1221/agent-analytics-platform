...
describe('SessionTable', () => {
    ...
    it('displays session data in correct format', () => {
        ...
        // Check date formatting - use getAllByText since multiple sessions may have same date
        const dateElements = screen.getAllByText('12/9/2025');
        expect(dateElements.length).toBeGreaterThan(0);

        // Check status badges - use getAllByText since "Completed" appears in filter button and badge
        ...
        // Check duration formatting - formatDuration doesn't show "0s" when there are minutes
    ...
    it('navigates to session detail when row is clicked', () => {
        ...
        expect(mockPush).toHaveBeenCalledWith('/dashboard/sessions/sess_001');
    });
    ...
    it('allows sorting by duration when header is clicked', () => {
        ...
        // Get initial order (sorted by started_at descending)
        const initialRows = screen.getAllByTestId(/^session-row-/);
        const initialOrder = initialRows.map((row) => row.getAttribute('data-testid'));

        // Click to sort by duration
        fireEvent.click(durationHeader);

        // Verify that order changed (sorting is working)
        const rowsAfterSort = screen.getAllByTestId(/^session-row-/);
        const newOrder = rowsAfterSort.map((row) => row.getAttribute('data-testid'));

        // Order should have changed
        expect(newOrder).not.toEqual(initialOrder);

        // Verify all sessions are still present
        expect(rowsAfterSort).toHaveLength(3);
        expect(newOrder).toContain('session-row-sess_001');
        expect(newOrder).toContain('session-row-sess_002');
        expect(newOrder).toContain('session-row-sess_003');
    });
    ...
    it('includes SessionFilters component', () => {
        ...
        // Check that filters are rendered (search was removed)
